...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"customUnitCost" object:nil];
}
...
-(void)setupBottomView:(NSArray *)products{
    ...
    for (int i=0; i<products.count; i++) {
        ...
        [productDirectMaterialCosts setupValue:product withTimeType:self.timeType];
...
-(void)setCustomUnitCost:(NSNotification*) notification{
    ...
    if ([[self.btnDate currentTitle] isEqualToString:@"今天"]||[[self.btnDate currentTitle] isEqualToString:@"昨天"]) {
        ...
        if ([value doubleValue]&&[value doubleValue]!=[Tool doubleValue:[[[self.data objectForKey:@"products"] objectAtIndex:self.selectIndex] objectForKey:@"customCost"]]) {
            ...
            [request setPostValue:dateString forKey:@"time"];
...
-(void)responseWithOtherCode{
    ...
    [self setupTopView];
    self.noDataView.frame = self.bottomView.bounds;
    [self.bottomView addSubview:self.noDataView];
    [self.noDataView.btn addTarget:self action:@selector(sendRequest) forControlEvents:UIControlEventTouchUpInside];
}
