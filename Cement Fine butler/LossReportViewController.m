...
@property (nonatomic,strong) ErrorOrNoDataView *noDataView;//请求出错或没有响应数据时响应页面
@end
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    if (self.dataArray.count>0) {
        ...
        if (products.count) {
            ...
            NSDictionary *configDict = @{@"tagName":@"(吨)",@"height":[NSNumber numberWithFloat:self.webView.frame.size.height],@"width":[NSNumber numberWithFloat:self.webView.frame.size.width],@"start_scale":[NSNumber numberWithInt:newMin],@"end_scale":[NSNumber numberWithInt:newMax],@"scale_space":[NSNumber numberWithInt:(newMax-newMin)/5]};
...
//            [self.view addSubview:self.messageView];
            ErrorOrNoDataView *noDataView = [[ErrorOrNoDataView alloc] initWithFrame:self.view.bounds];
