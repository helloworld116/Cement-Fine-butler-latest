...
-(void)showSelectedIndx:(NSInteger)selectedIndex{
    ...
    if (selectedIndex==0) {
        ...
        [self.btnRight setHighlighted:YES];
        self.btnLeft.enabled=NO;
        [self.btnLeft setHighlighted:NO];
        self.lblTypeName.text = @"原材料库存";
    }else if(selectedIndex==1){
        self.btnRight.enabled=NO;
        [self.btnRight setHighlighted:NO];
        self.btnLeft.enabled=YES;
        [self.btnLeft setHighlighted:YES];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    if (dataArray.count>0) {
        ...
        if (products.count) {
            ...
            NSDictionary *configDict = @{@"tagName":@"(吨)",@"height":[NSNumber numberWithFloat:webView.frame.size.height],@"width":[NSNumber numberWithFloat:webView.frame.size.width],@"start_scale":[NSNumber numberWithInt:newMin],@"end_scale":[NSNumber numberWithInt:newMax],@"scale_space":[NSNumber numberWithInt:(newMax-newMin)/5]};
