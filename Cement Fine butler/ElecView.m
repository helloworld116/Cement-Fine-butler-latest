...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"elecCustomUnitCost" object:nil];
}
...
-(void)setupBottomView{
    ...
    for (int i=0; i<self.products.count; i++) {
        ...
        productView = [[ElecProductView alloc] initWithFrame:CGRectMake(scrollViewSize.width*i, 0, scrollViewSize.width, self.bottomScrollView.contentSize.height) withData:product andTimeType:self.timeType];
...
-(void)dropDownDelegateMethod:(DropDownView *)sender{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        viewController.elecTimeType = sender.timeType;
...
@property (nonatomic, strong) IBOutlet UILabel *lblSettingLine;
@property (nonatomic, strong) IBOutlet UIImageView *imgViewSetting;
...
-(id)initWithFrame:(CGRect)frame withData:(NSDictionary *)data andTimeType:(int)timeType{
...
-(void)setupValue:(NSDictionary *)data{
    if (self.timeType==0||self.timeType==1) {
        self.lblSettingName.hidden = NO;
        self.lblSettingLine.hidden = NO;
        self.imgViewSetting.hidden = NO;
        [self.btnSetting addTarget:self action:@selector(showPopupView:) forControlEvents:UIControlEventTouchUpInside];
    }else{
        self.lblSettingName.hidden = YES;
        self.lblSettingLine.hidden = YES;
        self.imgViewSetting.hidden = YES;
        [self.btnSetting removeTarget:self action:@selector(showPopupView:) forControlEvents:UIControlEventTouchUpInside];
    }
