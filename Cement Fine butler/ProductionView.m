...
@property (nonatomic,strong) ErrorOrNoDataView *noDataView;
...
//            [self responseCode0WithNOData];
            ErrorOrNoDataView *noDataView = [[ErrorOrNoDataView alloc] initWithFrame:CGRectMake(self.selectIndex*kScreenWidth, 0, kScreenWidth, self.scrollVBttom.contentSize.height)];
            [self.scrollVBttom addSubview:noDataView];
            noDataView.lblMsg.text = @"没有满足条件的数据";
            noDataView.btn.hidden = YES;
...
//            sc.showsVerticalScrollIndicator = NO;
        ...
        ErrorOrNoDataView *noDataView = [[ErrorOrNoDataView alloc] initWithFrame:CGRectMake(self.selectIndex*kScreenWidth, 0, kScreenWidth, self.scrollVBttom.contentSize.height)];
        [self.scrollVBttom addSubview:noDataView];
        [noDataView.btn addTarget:self action:@selector(sendRequest) forControlEvents:UIControlEventTouchUpInside];
        self.noDataView = noDataView;
        [self bringSubviewToFront:noDataView];
    }
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    ...
    if (dataArray.count>0) {
        ...
        if (productsForSort.count) {
            ...
            if (min>100000) {
                ...
                for (int i=0;i<dataArray.count;i++) {
                    ...
                    if (value) {
                        ...
                        NSDictionary *reportDict = @{@"name":name,@"value":[Tool numberToStringWithFormatter:[NSNumber numberWithDouble:value]],@"color":color};
            ...
            NSDictionary *configDict = @{@"tagName":tagName,@"height":[NSNumber numberWithFloat:webView.frame.size.height],@"width":[NSNumber numberWithFloat:webView.frame.size.width],@"start_scale":[NSNumber numberWithInt:newMin],@"end_scale":[NSNumber numberWithInt:newMax],@"scale_space":[NSNumber numberWithInt:(newMax-newMin)/5]};
